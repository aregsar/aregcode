...
Another potential usage is to dynamically determine which pipeline middleware should apply to which routes. In fact this is done in other web frameworks such as Laravel.

Using endpoint routing also allows us to decouple the route matching logic from the MVC middleware, moving it into its own middleware. It allows the MVC middleware to focus on its responsablity of dispatching the request to the particular controller action method that is resolved by the endpoint routing middleware.
...
Endpoint route resolution was done in the old MVC middleware that is added to the pipeline using the AddMvc() extension method.

Going forward this will be done in its own endpoint routing middleware that we can add separately using its own AddRouting() extension method. 
